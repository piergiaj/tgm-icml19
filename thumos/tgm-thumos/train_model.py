...
def run_network(model, data, gpu, baseline=False):
    ...
    inputs = Variable(inputs.cuda(gpu)).squeeze(3).squeeze(3)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        model.load_state_dict(torch.load('models/'+args.model_file))

        print(datasets)
        lr = 2*0.01*batch_size/len(datasets['train'])
        print lr
        optimizer = optim.Adam(model.parameters(), lr=lr)
        lr_sched = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=5, verbose=True)

        
        run([(model,0,dataloaders,optimizer, lr_sched, args.model_file)], num_epochs=120)

    else:
        print 'Evaluating...'
        mdl = torch.load(args.rgb_model_file)
        model = models.get_hier(classes)
        rgb_model = nn.DataParallel(model)
        rgb_model.load_state_dict(mdl)
        print model
        ...
        flow_model = models.get_hier(classes)
        flow_model = nn.DataParallel(flow_model)
        flow_model.load_state_dict(torch.load(args.flow_model_file))
        ...
        for vid in rgb_results.keys():
            ...
            if vid in flow_results:
                ...
                o = (o[:o2.shape[0]]*0.5+o2*0.5)
                p = (p[:p2.shape[0]]*0.5+p2*0.5)
            apm.add(sigmoid(o), l)
            preds[vid] = (sigmoid(o)[:,:20].tolist(), fps)
